...
- [ ] T001 Record a green baseline: run `mvn -q verify` from the repository root and `npm test` in `bottin-client-ui/`, and note the counts so any later failure is attributable to this feature rather than inherited
- [ ] T002 Add the `nap-spring` dependency to `bottin-admin-ui/pom.xml`, matching the version `bottin-client-ui/pom.xml` already declares so the two applications cannot drift onto different protocol versions
