...
- [~] T010a Repair the pre-existing `bottin-it` context failure that blocks every integration test in the module. **Two layers fixed, one remains — the module is still red and this is not a settings defect.**

  **Root cause, established by bisecting each layer:** `BottinAutoConfiguration` carries `@ComponentScan` over `xyz.tcheeric.bottin.api` *from an `@AutoConfiguration` class*. Spring Boot's own documentation warns against this, and both remaining symptoms follow from it. It is registered through `AutoConfiguration.imports`, so it applies to every context with the starter on the classpath — which is why editing `TestApplication`'s own scan changed nothing but which bean collided first.

  - **Layer 1 — FIXED.** `BeanDefinitionOverrideException` on every repository: the auto-configuration's scan picks up `BottinApiApplication`, itself `@SpringBootApplication` with `@EnableJpaRepositories` over the same repository package, colliding with `TestApplication`'s identical declaration. `TestApplication` is now a bare `@SpringBootApplication`; the auto-configuration already contributes everything it was duplicating.
  - **Layer 2 — FIXED.** Actuator's `managementSecurityFilterChain` collided with `bottin-api`'s "any request" `defaultFilterChain`. `ManagementWebSecurityAutoConfiguration` is now excluded in `application-test.yml`.
  - **Layer 3 — OPEN.** Spring Boot's default `SpringBootWebSecurityConfiguration$SecurityFilterChainConfiguration` now collides with the same chain. `@ConditionalOnDefaultWebSecurity` should back off when a `SecurityFilterChain` bean exists, but `SecurityConfig` arrives via the auto-configuration's scan and is therefore registered *after* the condition is evaluated. Fixing it properly means stopping `BottinAutoConfiguration` from component-scanning — a production change to the published starter, with blast radius well beyond this feature.

  **Separately, and independently blocking:** `application-test.yml` sets `spring.flyway.enabled=false` with `ddl-auto: create-drop`, so **no integration test in this module ever runs a migration**. Hibernate builds the schema from the entities instead, without the seeded row or the `settings_singleton` constraint. `SettingsRepositoryIT` asserts what the migration produces, so it cannot pass here regardless of the context, and is `@Disabled` with this reason rather than left as an unexplained red test. Enabling Flyway for this class (`spring.flyway.enabled=true`, `ddl-auto=none`) is the smaller half of the fix.
